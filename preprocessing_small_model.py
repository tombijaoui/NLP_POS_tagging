...
from inference import get_confusion_matrix_and_accuracy, tag_all_test

WORD = 0
TAG = 1


def split_train_test(train_path, train_indices, test_indices):
    all_sentences = []
    train_sentences = []
    test_sentences = []
    ...
    with open(train_path) as f:
        ...
        for i in range(len(all_sentences)):
            if i in train_indices:
                train_sentences.append(all_sentences[i])

            if i in test_indices:
                test_sentences.append(all_sentences[i])

    with open("train_small_model.wtag", "w") as train_file:
        for line in train_sentences:
            train_file.write("".join(line))

    with open("test_small_model.wtag", "w") as test_file:
        for line in test_sentences:
            test_file.write("".join(line))

    return "train_small_model.wtag", "test_small_model.wtag"


def cross_validation(train_path, weights_path, predictions_path, threshold, lam, k):
    ...
    for i in range(k):
        ...
        train_path_small_model, test_path_small_model = split_train_test(train_path, train_indices, test_indices)
        statistics, feature2id = preprocess_train_small_model(train_path_small_model, test_path_small_model,
                                                              threshold)

        get_optimal_vector(statistics=statistics, feature2id=feature2id, weights_path=weights_path,
                           lam=lam)
        ...
        tag_all_test(test_path_small_model, pre_trained_weights, feature2id, predictions_path)

        w_confusion_matrix, accuracy = get_confusion_matrix_and_accuracy(test_path_small_model,
                                                                         predictions_path, feature2id)

        accuracies[i] = accuracy
        print(f"Accuracy score for fold {i + 1}: {accuracy:.2f}%")

        with open(train_path_small_model, 'r+') as file:
            file.truncate(0)

        with open(test_path_small_model, 'r+') as file:
            file.truncate(0)
    ...
    print(f"\nThe average accuracy is: {np.mean(accuracies):.2f}%")
    print("End of Cross Validation")
    print("---------------------------------------------------------------------------------------------------------\n")


class FeatureStatisticsSmallModel:
    def __init__(self):
    ...
    def get_word_tag_pair_count(self, train_path, test_path) -> None:
        """
...
class Feature2idSmallModel:
    def __init__(self, feature_statistics: FeatureStatisticsSmallModel, threshold: int):
        """
...
def preprocess_train_small_model(train_path, test_path, threshold):
    # Statistics
    statistics = FeatureStatisticsSmallModel()
    statistics.get_word_tag_pair_count(train_path=train_path, test_path=test_path)
    ...
    # feature2id
    feature2id = Feature2idSmallModel(statistics, threshold)
